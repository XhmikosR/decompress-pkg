...
function parseField(buffer, base, [fieldOffset, fieldLength], radix) {
  return Number.parseInt(buffer.subarray(base + fieldOffset, base + fieldOffset + fieldLength).toString('latin1'), radix);
}
...
function parseOdc(buffer) {
  ...
  while (offset + ODC.HEADER_SIZE <= buffer.length) {
    if (buffer.subarray(offset, offset + MAGIC_LENGTH).toString('latin1') !== ODC.MAGIC) {
      return null;
    }
    ...
    if (![mode, mtime, namesize, filesize].every(value => Number.isFinite(value)) || namesize < 1 || filesize < 0) {
      return null;
    }
    ...
    if (dataEnd > buffer.length || dataEnd <= offset) {
      return null;
    }
...
function parseNewc(buffer) {
  ...
  while (offset + NEWC.HEADER_SIZE <= buffer.length) {
    const magic = buffer.subarray(offset, offset + MAGIC_LENGTH).toString('latin1');
    ...
    const dataEnd = dataStart + filesize;
    const nextOffset = alignUp(dataEnd, NEWC.PAD_ALIGNMENT);

    // Reject sizes that overflow alignUp's int32 math or don't advance the offset
    if (dataEnd > buffer.length || dataStart < nameEnd || nextOffset <= offset) {
      return null;
    }
...
export function parseCpio(buffer) {
  ...
  const magic = buffer.subarray(0, MAGIC_LENGTH).toString('latin1');
  if (magic === ODC.MAGIC) {
