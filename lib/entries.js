...
async function getFileData(node, context) {
  const info = parseDataNode(getFirstChild(node, 'data'));
  if (info === null) {
    return null;
  }

  const {heapStart, input, maxFileSize} = context;
  ...
  if (encoding === 'application/x-gzip') {
    ...
    // <size> is untrusted, so the ceiling caps it too. maxOutputLength rejects 0,
    // so a declared-empty entry caps at 1 byte and fails the size check below.
    const declared = size >= 0 ? Math.max(size, 1) : maxFileSize;
    decompressed = await unzip(compressedContent, {maxOutputLength: Math.min(declared, maxFileSize)});
...
async function processNode(node, context, parentPath) {
  ...
  if (type === 'directory') {
    ...
    const children = await collectPkgEntries(node.file, context, currentPath);
  ...
  const data = await getFileData(node, context);
  if (data === null) {
    return [];
  }
...
// context: {heapStart, input, maxFileSize}
export async function collectPkgEntries(fileNodes, context, parentPath = '') {
  if (!Array.isArray(fileNodes)) {
    return [];
  }

  const nodePromises = fileNodes.map(node => processNode(node, context, parentPath));
