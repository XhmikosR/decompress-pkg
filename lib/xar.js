...
export function readHeader(buffer) {
  ...
  return {
    headerSize,
    tocLengthCompressed,
    tocLengthUncompressed,
  };
}

export async function parseToc(input, header, maxTocSize) {
  ...
  // Bound inflation to the declared length, or to the ceiling if the header omits it
  const maxOutputLength = Math.min(header.tocLengthUncompressed || maxTocSize, maxTocSize);
  const tocBuffer = await unzip(tocCompressed, {maxOutputLength});
